...
# 1. Add a new dict to ACHIEVEMENTS_DEF list below.
# 2. Required fields:
#    - "id"            : unique snake_case identifier (e.g., "my_new_ach")
#    - "title"         : display name in Russian
#    - "description"   : short description in Russian
#    - "goal"          : numeric goal (1 for complex, N for stat)
#    - "stat_key"      : one of the stat fields (see Stats class) or "_complex"
#    - "category"      : one of ACH_CATEGORIES keys (EDITING/MATERIALS/GEO_NODES/TIME/RENDERING)
#    - "check_type"    : "stat" (compare stat >= goal) or "complex" (scene-state check)
#    - "reward_type"   : "tutorial", "material", "mesh", "geo_nodes", or "none"
#    - "reward_data"   : dict with reward details (see examples below)
#    - "reward_category": one of REWARD_CATEGORIES keys (for Storage tab grouping)
#    - "icon_gray"     : filename for locked icon (place in ~/BlenderAchievements/textures/)
#    - "icon_color"    : filename for unlocked icon
#    - "lesson_id"     : linked lesson ID or None
#
# 3. For complex achievements, also add:
#    - "complex_id"    : unique id used in _check_complex() function
#    - "steps"         : list of step dicts for multi-step progress display
#      Each step: {"label": "Step name", "check": "function_name_or_key"}
#    Then implement the check logic in _check_complex() and _check_complex_step().
#
# HOW TO REMOVE AN ACHIEVEMENT:
# 1. Delete the dict from ACHIEVEMENTS_DEF.
# 2. If it was a complex achievement, remove its check logic from _check_complex().
# 3. Previously earned progress for removed achievements stays in JSON but is ignored.
#
# HOW TO ADD REWARD MATERIALS:
# 1. Create a .blend file with the material/mesh/geo_nodes group.
# 2. Place it in ~/BlenderAchievements/rewards/ folder.
# 3. Set reward_data fields:
#    For materials: {"name": "MAT_NAME_IN_BLEND", "description": "Display name",
#                    "blend_file": "rewards/filename.blend"}
#    For meshes:    {"name": "OBJ_NAME_IN_BLEND", "description": "Display name",
#                    "blend_file": "rewards/filename.blend"}
#    For geo_nodes: {"name": "NODEGROUP_NAME_IN_BLEND", "description": "Display name",
#                    "blend_file": "rewards/filename.blend"}
# 4. The "name" must match exactly the datablock name inside the .blend file.
# 5. If .blend is missing, addon creates a placeholder (colored material / icosphere / empty modifier).
#
# HOW TO ADD NEW ICONS:
# 1. Create two PNG images: achievement_id_gray.png and achievement_id_color.png
# 2. Recommended size: 128x128 px (displayed at 100x100 in UI)
# 3. Place both files in ~/BlenderAchievements/textures/
# 4. Set icon_gray and icon_color fields to the filenames.
...
# 1. Add a dict to LESSONS_DEF with:
#    - "id"          : unique identifier (e.g., "lesson_my_topic")
#    - "title"       : display name in Russian
#    - "description" : short description
#    - "category"    : one of LESSON_CATEGORIES keys
#    - "url"         : link to tutorial (YouTube, etc.)
#    - "icon"        : icon filename in textures/ folder
# 2. Link achievements to lessons via "lesson_id" field in ACHIEVEMENTS_DEF.
