...
`Achievements` is currently a Blender add-on with a root runtime entrypoint, pure catalog/event/lifecycle/persistence helper modules, and a byte-identical tracked duplicate of the root entrypoint.
...
- Runtime surfaces: Blender handlers, timers, Scene properties, GPU draw UI, lifecycle helper wiring, activity/session tracking, and reward loading from `.blend` assets.
